 ...
 * dentro de los documentos de la propuesta (documentType "Propuesta cía").
 * Idempotente: reemplaza el archivo del documento auto-guardado si ya existe.
 */
async function storeSentPdfAsDocument(
  db: Db,
  ...
  const fileName = `Propuesta N° ${proposalNumber} (enviada a la cía).pdf`;
  ...
  if (existing) {
    ...
    if (existing.storageKey) {
      ...
      await deleteStoredFile(existing.storageKey).catch(() => {});
    }
    return;
  }
...
export async function sendProposalByEmailAction(
  ...
  if (options.markOnly || !recipient) {
    ...
    try {
      ...
      let pdf: Buffer | Uint8Array | null = stored?.pdfBytes
        ? Buffer.from(stored.pdfBytes)
        : null;
      if (!pdf) {
        const data = await buildProposalPdfData(db, proposalId);
        if (data) pdf = await renderProposalPdf(data);
      }
      if (pdf) {
      ...
      console.error("No se pudo guardar el PDF como documento", e);
    }
    revalidatePath(`/propuestas/${proposalId}`);
  ...
  await storeSentPdfAsDocument(
    ...
    pdfBuffer,
    ctx.userId,
  ).catch((e) => console.error("No se pudo guardar el PDF como documento", e));
  revalidatePath(`/propuestas/${proposalId}`);
